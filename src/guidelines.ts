/**
 * Generative UI design guidelines content.
 *
 * Each module covers a specific visualization category. The assembler
 * deduplicates shared sections when multiple modules are requested.
 */
...
export const AVAILABLE_MODULES = Object.keys(MODULE_SECTIONS) as readonly string[];

export type ModuleName = 'interactive' | 'chart' | 'mockup' | 'art' | 'diagram';

/**
 * Assemble full guidelines from requested module names.
 * Deduplicates shared sections (e.g. Core Design System appears once
 * even when multiple modules are requested).
 */
...
/**
 * System prompt snippet for hosts that inject widget capability
 * into their system prompt. Contains all hard constraints needed
 * for valid widget output even without calling the tool.
 */
export const SYSTEM_PROMPT = `<widget-capability>
...
## Design specs
Call \`load_ui_guidelines\` before your first widget to load detailed design specs.
